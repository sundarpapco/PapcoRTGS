package com.papco.sundar.papcortgs.ui.screens.party.addSender
...
@Composable
fun AddEditSenderScreen(
    state:AddEditPartyState,
    onFormSubmit:()->Unit
){
    ...
    Scaffold(
        topBar = {
            RTGSAppBar(
                title = stringResource(id = R.string.create_sender),
                isBackEnabled = true,
                onBackPressed = {

                },
                optionsMenu = {
                    OptionsMenu(menuItems = listOf(
                        MenuAction(imageVector = Icons.Filled.Done, label = stringResource(id = R.string.done))
                    )) {
...
private fun PreviewAddEditSenderScreen(){
    ...
    RTGSTheme {
        AddEditSenderScreen(state = state) {
